window.onload = function() {
	...
	bLMemberId = getParameterByName("bLMemberId");
	downPageNum = (startPageNum!="")?startPageNum:1;
	upPageNum = (startPageNum!="")?startPageNum:1;
	bLMemberId = (!isEmpty(bLMemberId))?bLMemberId:"";
	arrMethod = (!isEmpty(arrMethod))?arrMethod:"p";
	getBoardList(downPageNum,showBoardId,true,arrMethod,bLMemberId);
	
	$("#arraySelect").on("change",function() { //순서정렬 기능
		let method = $("#arraySelect option:selected").val()
		if(method == "p") {
			location.replace("list?arrMethod=p&bLMemberId="+bLMemberId);
		} else {
			location.replace("list?arrMethod=u&bLMemberId="+bLMemberId);
		}
	});
}
...
$(window).scroll(function() {
	...
	if(currentScrollTop - lastScrollTop > 0) { // 다운스크롤
		...
		if(!boardListDownEnd && (currentScrollTop >= $(document).height() - $(window).height() - 500)) {
			...
			downPageNum++;
			getBoardList(downPageNum,"",true,arrMethod,bLMemberId);
		}
	} else { // 업스크롤
		lastScrollTop = currentScrollTop;
		if(!boardListUpEnd && currentScrollTop < 800) {
			...
			upPageNum--;
			getBoardList(upPageNum,"",false,arrMethod,bLMemberId);
		}
	}
});


function getBoardList(pageNum,showBoardId,toggle,arrMethod,bLMemberId) {
	if(!ajaxClick) {
		return false;
	}
	downScrolling = false;
	...
	ajaxClick = false;
	arrMethod = (!isEmpty(arrMethod))?arrMethod:"p";
	var data = {pageNum:pageNum,arrMethod:arrMethod,bLMemberId:bLMemberId};
	var boardVal = [];
	var boardUDateVal = [];
