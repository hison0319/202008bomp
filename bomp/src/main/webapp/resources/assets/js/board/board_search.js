window.onload = function() {
	...
	getBoardList(downPageNum,showBoardId,true,keyword,arrMethod);
	
	$("#arraySelect").on("change",function() { //순서정렬 기능
		let method = $("#arraySelect option:selected").val()
		if(method == "p") {
			location.replace("list?arrMethod=p&keyword="+keyword);
		} else {
			location.replace("list?arrMethod=u&keyword="+keyword);
		}
	});
}
...
$(window).scroll(function() {
	...
	if(currentScrollTop - lastScrollTop > 0) { // 다운스크롤
		...
		if(!boardListDownEnd && (currentScrollTop >= $(document).height() - $(window).height() - 500)) {
			...
			downPageNum++;
			getBoardList(downPageNum,"",true,keyword,arrMethod);
		}
	} else { // 업스크롤
		lastScrollTop = currentScrollTop;
		if(!boardListUpEnd && currentScrollTop < 800) {
			...
			upPageNum--;
			getBoardList(upPageNum,"",false,keyword,arrMethod);
		}
	}
});


function getBoardList(pageNum,showBoardId,toggle,keyword,arrMethod) {
	if(!ajaxClick) {
		return false;
	}
	downScrolling = false;
	...
	var data = {pageNum:pageNum,keyword:keyword,arrMethod:arrMethod};
	var boardVal = [];
	var boardUDateVal = [];
		...
		success : function(data) {
	       	if (data.code == "OK") {
				...
				for(let i=0; i<boardVal.length; i++) {
					...
					let htmlBoMemberNick = "<a href='javascript:return false;' onclick='showMember("+boMemberId+")'><span style='font-weight:bold;'>"+boMemberNick+"</span></a>";
					...
					if(boardTag!=null) {
						...
						if(tag1!=null) {
							...
							if(keyword==tag1) {
									htmlTag1 = `<li><a href="javascript:return false;" onclick="searchKeyword('`+tag1+`')"><span class="tag" style="color: #3CA03C; font-size:1.1em;">#`+tag1+`</span></a></li>`;						
							}
						...
						if(tag2!=null) {
							...
							if(keyword==tag2) {
									htmlTag2 = `<li><a href="javascript:return false;" onclick="searchKeyword('`+tag2+`')"><span class="tag" style="color: #3CA03C; font-size:1.1em;">#`+tag2+`</span></a></li>`;
							}
							for(let j=0;j<memberTagArr.length; j++) {
						...
						if(tag3!=null) {
							...
							if(keyword==tag3) {
									htmlTag3 = `<li><a href="javascript:return false;" onclick="searchKeyword('`+tag3+`')"><span class="tag" style="color: #3CA03C; font-size:1.1em;">#`+tag3+`</span></a></li>`;
							}
							for(let j=0;j<memberTagArr.length; j++) {
						...
						if(tag4!=null) {
							...
							if(keyword==tag4) {
									htmlTag4 = `<li><a href="javascript:return false;" onclick="searchKeyword('`+tag4+`')"><span class="tag" style="color: #3CA03C; font-size:1.1em;">#`+tag4+`</span></a></li>`;
							}
							for(let j=0;j<memberTagArr.length; j++) {
						...
						if(tag5!=null) {
							...
							if(keyword==tag5) {
									htmlTag5 = `<li><a href="javascript:return false;" onclick="searchKeyword('`+tag5+`')"><span class="tag" style="color: #3CA03C; font-size:1.1em;">#`+tag5+`</span></a></li>`;
							}
							for(let j=0;j<memberTagArr.length; j++) {
