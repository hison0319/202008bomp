...
<script src="/js/board/posting.js"></script>
...
<body onload="noBack();" onpageshow="if(event.persisted) noBack();"
	...
	<section class="wrapper style">
		<div class="inner">
			<div class="posting">
				<form action="posting" name="postingForm" method="POST"
					enctype="multipart/form-data">
					<div class="textWindow">
						<select name="categoryName" id="category">
							<option value="0">- 카테고리 -</option>
							<option value="자연">자연</option>
							<option value="사물">(인위적)사물</option>
							<option value="풍경">풍경</option>
							<option value="인물">인물</option>
							<option value="기타">기타(분류에 없는)</option>
							<option value="토론방">토론방</option>
							<c:if test="${sessionScope.member.memberId <= 5 and sessionScope.member.memberId != 0}">
								<option value="공지사항">공지사항</option>
							</c:if>
						</select>
						<div class="col-6 col-12-xsmall">
							<input type="hidden" name="memberId"
								value="${sessionScope.member.memberId}" readonly /> <input
								type="text" name="memberNick" id="memberNick"
								value="${sessionScope.member.memberNick}" readonly /> <input
								type="text" name="title" id="title" value=""
								placeholder="제목 (단어)" maxlength="100" />
						</div>
						<div class="col-12">
					...
					<input type="text" name="boardTagStr" id="boardTag" value=""
						placeholder="태그를 #으로 구분해서 넣어주세요!(5개 이하)" /> <br>
					<div class="fileInput">
						이미지 업로드(1장)&nbsp;<span class="icon fas fa-paperclip"></span>&nbsp;&nbsp;&nbsp;<input
							type="file" class="imgAttachment" name="imageFile"
							accept="image/gif, image/jpeg, image/jpg, image/x-png"
							aria-label="이미지업로드" data-tip="이미지업로드‬">&nbsp;&nbsp;최대 2MB
					</div>
					<br> <input type="text" name="imgAlt" id="imgAlt" value=""
						placeholder="시각장애인을 위해 간단한 이미지 설명을 해주세요." />
				</form>
				<ul class="actions fit">
					<li><button class="upLoad" aria-label="게시하기" data-tip="게시하기"
							onclick="sumbmitCheck()">게시하기</button></li>
					<li><button class="cancel" aria-label="취소하기" data-tip="취소하기"
							onclick="cancel()">다시쓰기</button></li>
				</ul>
			</div>
		</div>
	</section>
